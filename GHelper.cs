...
namespace Gunplay
{
    public static class GHelper
    {
		...
		public static void AddPart(PartType partType)
		{
			Gunplay.parts.Add(partType.key, partType);
		}

		public static PartType GetPartType(string key)
		{
		...
		{
			if (item.type != ModContent.ItemType<ConstructTool>())
			{
			...
			{
				if ((item.modItem as ConstructTool).effects.ContainsKey(effect.Key))
				{
					(item.modItem as ConstructTool).effects[effect.Key] = Math.Min((item.modItem as ConstructTool).effects[effect.Key] + effect.Value, GetEffectType(effect.Key).maxLevel);
				}
				else
				{
					(item.modItem as ConstructTool).effects.Add(effect.Key, effect.Value);
				}
			}
		}
		...
		public static int CheckEffect(Item item, string effect)
		{
			...
			if ((item.modItem as ConstructTool).effects.ContainsKey(effect))
			{
				return (item.modItem as ConstructTool).effects[effect];
			}
			else
			{
				return -1;
		...
		public static int PowerFromParts(List<string> keys)
		{
			if (keys.Count == 0)
			{
				return 0;
			...
			{
				powers.Add(GetPartType(key).power);
			}
		...
		{
			PartType partType = GetPartType(key);
			return partType.materialName + " " + partType.type;
		}

		public static void SetDefaults(Item item)
		{
			ConstructTool cTool = item.modItem as ConstructTool;
			...
			int power = PowerFromParts(cTool.parts);
			int headPower = GetPartType(cTool.parts[cTool.parts.Count - 1]).power; //Get power of last item, which should be the head

			//int headPower = GetPartType(cTool.parts[0]).power;
			item.damage = power;
			item.knockBack = 1f;
			item.useTime = 30;
